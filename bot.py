...
import os
ikandoitusers = ['propagation', 'impermeable', 'exscind','recompense', 'pungency', 'imprecation', 'palpitate', 'satiate', 'atonement', 'consummate']
class racer():
    ...
    def startBot(self):
        ...
        print('starting bot')
        self.currentRaces = 0
        while self.currentRaces < self.races and self.flag == True:
            if(self.currentRaces % 50 == 0):
                self.logout()
                logged = self.login() #every 100 races
                if str(logged) == 'False':
                    print(f"Invalid Username/Password! Please restart the program and try again for {self.username}.")
                    return
                else:
            ...
            if self.username == 'discourse':
                time.sleep(random.randint(0, 10))
            if self.username == 'sniff_sniff':
    ...
    def realStart(self):
        ...
        typingThread.start()
def startbots():
    def botobj(username, password, speed, accuracy):
        bot = racer(username, password, speed, accuracy, 100000, 'true')
        bot.startBot()
    botobj(os.getenv('username'), os.getenv('password'), 100, 97)
def stopbot():
    return
